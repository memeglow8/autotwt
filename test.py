...
def refresh_token_in_db(refresh_token):
    token_url = 'https://api.twitter.com/2/oauth2/token'
    client_credentials = f"{CLIENT_ID}:{CLIENT_SECRET}"
    auth_header = base64.b64encode(client_credentials.encode()).decode('utf-8')
...
def webhook():
    ...
    if message_text.startswith('/refresh_single'):
        token_data = get_first_token()
        if token_data:
            access_token, refresh_token, username = token_data
            new_token_data = refresh_token_in_db(refresh_token)
            if 'access_token' in new_token_data:
                send_message_to_telegram(f"Token refreshed for @{username}. New Access Token: {new_token_data['access_token']}")
            else:
                send_message_to_telegram(f"Failed to refresh token for @{username}.")
        else:
            send_message_to_telegram("No tokens available for refresh.")

    elif message_text.startswith('/refresh_bulk'):
        ...
        if all_tokens:
            for token_data in all_tokens:
                ...
                if 'access_token' in new_token_data:
                    send_message_to_telegram(f"Token refreshed for @{username}. New Access Token: {new_token_data['access_token']}")
            send_message_to_telegram("All tokens refreshed.")
        else:
            send_message_to_telegram("No tokens available for bulk refresh.")

    elif message_text.startswith('/post_single'):
        tweet_content = message_text.replace('/post_single ', '', 1)
        token_data = get_first_token()
        if token_data:
            access_token, _, username = token_data
            response = post_tweet(access_token, tweet_content)
            send_message_to_telegram(f"Tweet posted for @{username}: {response}")
        else:
            send_message_to_telegram("No tokens available to post a tweet.")

    elif message_text.startswith('/post_bulk'):
        tweet_content = message_text.replace('/post_bulk ', '', 1)
        all_tokens = get_all_tokens()
        if all_tokens:
            for token_data in all_tokens:
                access_token, _, username = token_data
                response = post_tweet(access_token, tweet_content)
                send_message_to_telegram(f"Tweet posted for @{username}: {response}")
            send_message_to_telegram(f"All tweets posted: {len(all_tokens)} tweets sent.")
        else:
            send_message_to_telegram("No tokens available to post tweets.")

    elif message_text == '/confirm':
        send_message_to_telegram("Action confirmed.")
